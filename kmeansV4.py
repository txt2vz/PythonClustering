...
# Feature Extraction
    ...
    maxFeature = 1000
    useHashingPipeline = False
   # useIDF = True

    if useHashingPipeline:
        hasher = HashingVectorizer(
        n_features=maxFeature,
        stop_words="english",
...
# -------------------
       ...
       # print("NMI: %.3f" % nmi)
        ...
        filePath = "results/resultsKpython.csv"
        resultsFile = open(filePath, "a")

        if os.path.getsize(filePath) == 0:
            resultsFile.write("index, km_v, km_h, km_c, km_adjustRand, ag_v, ag_adjustedRand, sc_v, sc_adjustedRand, maxFeature, numDocs, useHashingPipeLine, date \n")

        resultsFile.write(f"{collectionName}, {km_v}, {km_h}, {km_c}, {km_adjustedRand}, {ag_v}, {ag_adjustedRand}, {sc_v}, {sc_adjustedRand}, {maxFeature}, {numDocs}, {useHashingPipeline}, {datetime.now()}  \n")    

        print()
        resultsFile.close()

print("Total time done in %0.3fs" % (time() - t0))
